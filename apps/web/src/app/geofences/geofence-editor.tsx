...
export function GeofenceEditor({ initial, canWrite }: { initial: GeofenceDto[]; canWrite: boolean }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, display: "grid", gridTemplateColumns: "minmax(0, 320px) minmax(0, 1fr)", fontFamily: "system-ui" }} className="rio-split">
      <style>{`@media (max-width: 760px){ .rio-split{ grid-template-columns: minmax(0,1fr) !important; grid-template-rows: 55vh 45vh; } .rio-split > aside{ order: 2 } }`}</style>
